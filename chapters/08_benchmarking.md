...
> розрив оптимальності (optimality gap), що також враховує якість межі. Ці метрики дають об’єктивну
...
4. **Аналіз масштабованості: як продуктивність змінюється з розміром екземпляра.**
   ...
   екземпляра вона здатна розв’язати оптимально і як далі росте розрив оптимальності,
   **split plots** — хороший вибір. Вони показують час для розв’язаних
   екземплярів і розрив оптимальності для тих, що перевищили ліміт часу, даючи цілісне
...
### Швидке порівняння з базовою моделлю за допомогою діаграм розсіювання
...
**розрив оптимальності** замість часу.
...
| :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------: |
| Розділений графік (split plot), що ілюструє час роботи (для розв’язаних екземплярів) і розрив оптимальності (для нерозв’язаних). Вісь y поділено на дві області: одна показує фактичні часи для екземплярів, розв’язаних у межах ліміту, а інша — розрив оптимальності для екземплярів, що перевищили ліміт. |
